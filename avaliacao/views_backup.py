    ...
    if request.method == 'POST':
        ...
        if turma_id and disciplina_id and nome and data_aplicacao:
            try:
                ...
                # Buscar ou criar período padrão
                divisao_periodo, created = DivisaoPeriodoLetivo.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'ordem': 1,
                        'data_inicio': f'{turma.periodo_letivo}-01-01',
                        'data_fim': f'{turma.periodo_letivo}-03-31',
                        'ativo': True
                    }
                )
...
def criar_avaliacao(request, turma_id=None):
    ...
    if request.method == 'POST':
        ...
        if all([turma_id, disciplina_id, nome, data_aplicacao]):
            turma_obj = get_object_or_404(Turma, pk=turma_id)
            disciplina = get_object_or_404(Disciplina, pk=disciplina_id)
            
            # Se não foi fornecido período, criar um padrão
            if divisao_periodo_id:
                ...
                divisao_periodo, created = DivisaoPeriodoLetivo.objects.get_or_create(
                    ...
                    defaults={
                        'tipo_divisao': 'BIMESTRE',
                        'ordem': 1,
                        'data_inicio': f'{turma_obj.periodo_letivo}-01-01',
                        'data_fim': f'{turma_obj.periodo_letivo}-03-31',
                        'ativo': True
                    }
                )
            
            # Se tipo_avaliacao não fornecido, usar o primeiro disponível
            if tipo_avaliacao_id:
                tipo_avaliacao = get_object_or_404(TipoAvaliacao, pk=tipo_avaliacao_id)
            else:
                tipo_avaliacao = TipoAvaliacao.objects.filter(ativo=True).first()
                if not tipo_avaliacao:
                    # Criar tipo padrão se não existir
                    tipo_avaliacao = TipoAvaliacao.objects.create(
                        nome="Prova",
                        descricao="Prova padrão",
                        ativo=True
                    )
...
def diario_eletronico_dashboard(request):
    ...
    """
    # Buscar turmas do usuário (se professor) ou todas (se coordenador/admin) - OTIMIZADO
    if request.user.groups.filter(name='Professor').exists():
        turmas_ids = DiarioEletronico.objects.filter(
            # Para professor: apenas turmas onde ele tem diários
        ).values_list('turma', flat=True).distinct()
        turmas = Turma.objects.filter(id__in=turmas_ids).select_related().prefetch_related('enturmacoes__aluno')
    else:
        turmas = Turma.objects.all().select_related().prefetch_related('enturmacoes__aluno')[:20]  # Limit para performance
...
def diario_eletronico_turma(request, turma_id):
    ...
    """
    turma = get_object_or_404(Turma.objects.select_related(), pk=turma_id)
    
    # Buscar diários da turma - OTIMIZADO
    diarios = DiarioEletronico.objects.filter(turma=turma, ativo=True).select_related('disciplina', 'turma')
    
    # Alunos da turma - OTIMIZADO  
    alunos = turma.get_alunos_enturmados().select_related()
...
def get_anos_series_por_tipo(request):
    """
    Endpoint AJAX para retornar anos/séries baseados no tipo de ensino
    """
    if request.method == 'GET':
        tipo_ensino = request.GET.get('tipo_ensino', '')
        opcoes = Turma.get_anos_series_por_tipo(tipo_ensino)
        
        # Formatar para JSON
        data = {
            'opcoes': [{'value': opcao[0], 'text': opcao[1]} for opcao in opcoes]
        }
...
def fazer_chamada_diario(request, diario_id):
    """
    Fazer chamada para um diário eletrônico específico
    """
    diario = get_object_or_404(DiarioEletronico, pk=diario_id)
    
    # Data da chamada (hoje por padrão, ou data especificada)
    data_chamada = request.GET.get('data', datetime.now().strftime('%Y-%m-%d'))
    data_chamada_obj = datetime.strptime(data_chamada, '%Y-%m-%d').date()
    
    # Alunos do diário
    alunos = diario.get_alunos().select_related()
    ...
    registros_dict = {registro.aluno.id: registro for registro in registros_existentes}
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f'Chamada de {data_chamada_obj.strftime("%d/%m/%Y")} realizada com sucesso!')
        return redirect('avaliacao:diario_eletronico_turma', turma_id=diario.turma.id)
    ...
    context = {
        ...
        'data_chamada': data_chamada,
        'data_chamada_obj': data_chamada_obj,
        'registros_dict': {str(k): v.__dict__ for k, v in registros_dict.items()},
    }
...
def lancar_notas_diario(request, diario_id):
    """
    Lançar notas para um diário eletrônico específico
    """
    diario = get_object_or_404(DiarioEletronico, pk=diario_id)
    ...
    # Se não há período selecionado, usar o primeiro disponível
    if not periodo_selecionado and periodos.exists():
        periodo_selecionado = periodos.first()
    
    # Alunos do diário
    alunos = diario.get_alunos().select_related()
    
    # Notas existentes para o período selecionado
    notas_existentes = []
    notas_dict = {}
    if periodo_selecionado:
        notas_existentes = RegistroNota.objects.filter(
            diario=diario,
            divisao_periodo=periodo_selecionado
        ).select_related('aluno')
        notas_dict = {nota.aluno.id: nota for nota in notas_existentes}
    
    # Conceitos se a disciplina avaliar por conceito
    conceitos = []
    usa_conceitos = diario.disciplina.avalia_por_conceito
    if usa_conceitos:
        conceitos = Conceito.objects.filter(ativo=True).order_by('valor_numerico')
    ...
    if request.method == 'POST' and periodo_selecionado:
        ...
        for aluno in alunos:
            if usa_conceitos:
                ...
                conceito = None
                if conceito_id:
                    conceito = get_object_or_404(Conceito, pk=conceito_id)
            else:
                nota_value = request.POST.get(f'nota_{aluno.id}')
            ...
            registro, created = RegistroNota.objects.get_or_create(
                ...
                divisao_periodo=periodo_selecionado,
                defaults={
                    'nota': float(nota_value) if not usa_conceitos and nota_value else None,
                    'conceito': conceito if usa_conceitos else None,
                    'observacoes': observacoes,
                    'usuario_lancamento': request.user
                }
            )
            ...
            if not created:
                if usa_conceitos:
                    ...
                    registro.nota = conceito.valor_numerico if conceito else None
                else:
                    registro.nota = float(nota_value) if nota_value else None
                registro.observacoes = observacoes
                registro.save()
        
        messages.success(request, f'Notas do {periodo_selecionado.nome} lançadas com sucesso!')
        return redirect('avaliacao:lancar_notas_diario', diario_id=diario.id)
    ...
    context = {
        ...
        'periodos': periodos,
        'periodo_selecionado': periodo_selecionado,
        'notas_dict': {str(k): v.__dict__ for k, v in notas_dict.items()},
        'conceitos': conceitos,
        'usa_conceitos': usa_conceitos,
    }
